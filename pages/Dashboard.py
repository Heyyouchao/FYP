...
# LIVE (raw)
df_live_raw = pd.read_csv("data/merged/multi_class_dataset.csv")
df_live_raw.columns = df_live_raw.columns.str.strip()
...
# df_s = df_debug[df_debug["marker"] == scenario].reset_index(drop=True)
...
# ============================================================
    ...
    from engine.preprocessing import clean_live_row
    row_clean = clean_live_row(row_raw, df_debug)
...
# 1. PHYSICAL MODEL
# ============================================================
raw_scores, norm_scores, state = classify_relay_scores(row_clean)
...
# 4. GRID
# ============================================================
physical_layer = process_event(norm_scores)
...
with col_left:
    ...
    with st.container():
        ...
        if "error" in data:
            ...
            st.write(f"Disturbance: {raw_scores[current_display_relay]:.2f}")
...
# ============================================================
with col_right:
    col_centre, col_alert = st.columns([3, 1])
    # st.markdown('<div class="main-content">', unsafe_allow_html=True)
    with col_centre:
        with st.container():
            st.markdown('<div class="card-anchor"></div>', unsafe_allow_html=True)
        ...
        # =========================
            with col_table:
                ...
                components = [
                    ...
                    ("🛡 IDS", "Alert" if result["Final_binary"] == 1 else "Monitoring", None),
